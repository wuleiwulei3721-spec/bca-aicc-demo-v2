...
- `DEV_LOG.md`
- `.codex-backup/key-prompts.md`
- `.codex-backup/context-snapshot-2026-06-06-1046.md`
- `.codex-backup/current-todo-2026-06-06-1046.md`
- `.codex-backup/page-state-2026-06-06-1046.md`

修改原因：

- 用户希望把本地内部 Call Management / Routing Config 改造集中到一条清晰分支，并清理不再需要的本地分支，避免客户安全分支、管理分支和临时 worktree 混淆。

修改结果：

- 从最新 `main` 创建 `codex/admin-config-latest` 并合入 `codex/routing-vdn-sites-remove-status`。
- 保留 `main` 上已发布客户功能：AUX 原因选择弹框、PSTN 客户身份刷新、Live Chat Current 清空空态。
- 恢复内部管理菜单和路由：`Call Management`、`Routing Config` 可在本地管理分支访问。
- 合并 Busy Reason store：AUX 弹框只展示启用原因，内部 Busy Reason Management 页面继续支持编辑、唯一默认值和状态维护。
- 保留 Routing Config 本地改造：VDN / Access Sites / Skill Queues 去状态字段，Skill Queues 和 Channel Business Config 使用 `Default 24/7` 默认工作时间展示。
...
- `npm run build` 通过；仍只有既有 Vite chunk size warning。
- `git diff --check` 通过。
- Browser smoke check `/`：展开侧栏后可见 `Call Management` 与 `Routing Config`。
- Browser smoke check `/design-system`、`/call-management/global-control-configuration`、`/call-management/busy-reasons`、`/routing-config/channels`、`/routing-config/skill-queues`、`/routing-config/working-time-plans` 均正常加载。
- Browser smoke check `Skill Queues`：列表可见 `Default 24/7`，未出现 `Default 24x7`。
- Browser smoke check `Working Time Plans`：未出现真实 `Default 24/7` 或 `Default 24x7` 记录。
- Browser smoke check AUX：签入后菜单只显示单个 `AUX` 与 `Sign Out`，不显示 `AUX - Ibadah` / `AUX - Makan` 直接菜单项；AUX 弹框只列出启用原因 `Ibadah`、`Makan`，不显示禁用的 `Training`。

回滚说明：

- 如需回到客户安全版本，切回 `main`；不要把 `codex/admin-config-latest` 合并或发布到 Production。
- 如需回滚本次整合，可删除本地 `codex/admin-config-latest`，重新从 `main` 或旧管理分支创建工作分支。

当前风险点：

- 本分支包含内部管理菜单，不适合直接发布给客户。
- 管理分支与客户分支长期并行后仍需在每次客户功能发布后手动同步一次。
